...
public class VectorTilesController : MapTilesController<VectorTile>
{
    ...
    public VectorTilesController(VectorMapStyle styles) : base(
        styles.Sources.Count == 0 || styles.Sources[0].Url is null ? "unknown url" : styles.Sources[0].Url!, 100)
    {
        styles1 = styles;
        ...
        if (s is not null && s.StartsWith("pmtiles://"))
        {
            ...
            PMTiles = PMTilesReader.FromUrl(s[10..].Replace("/{z}/{x}/{y}",
                "")); // PMTilesReaderを作成する際にWebリクエストを行うため、非同期で実行
        }
    }

    internal class VectorTileRequest(
        SKPoint point,
        TilePoint tilePoint,
        string url,
        VectorMapStyle styles,
            ...
            tilePoint, url)
    {
        ...
        public override VectorTile GetAndParse(Stream? data) =>
            data is null ? new VectorTile(null) : new VectorTile(styles.ParsePaths(data, TilePoint));
